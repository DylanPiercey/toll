...
function Connection (proxy, opts) {
	// Ensure proxy provided.
	if (!proxy) throw new TypeError("Toll: You must specify the proxy server address.");

	// Extract options and set defailts.
	opts            = opts || {};
	this.connect    = this.connect.bind(this);
	this._heartbeat = opts.heartbeat || 15000;
	this._address   = parseAddress(proxy);
	this._servers   = [];
}
...
function connect () {
	...
	self._interval = setInterval(self.connect, self._heartbeat);
...
function parseAddress (address) {
	...
	if (typeof address.address === "function") {
